...
import { disableScrollSnap, enableScrollSnap, isScrollSnapDisabled } from './scrollSnap'
...
const SIDEBAR_LAYOUT_LOCK_MS = Math.round(transition.duration * 1000) + 120
const SIDEBAR_LAYOUT_TOKEN = 'sidebar-layout'
...
function sectionIndexAtScrollTop(sections, scrollTop) {
  const index = sections.findIndex((section, i) => {
    ...
    return (
      scrollTop >= section.offsetTop - SECTION_EDGE_TOLERANCE &&
...
export default function App() {
  const { sections, loading, error } = useContent()
  const [activeSection, setActiveSection] = useState('')
  const scrollRef = useRef(null)
  const lockedRef = useRef(false)
  const wheelLockedRef = useRef(false)
  const lockTimeoutRef = useRef(null)
  const sidebarLayoutTimeoutRef = useRef(null)
  ...
  useEffect(() => {
    ...
    sectionElsRef.current = [...container.querySelectorAll('section[id]')]
  }, [sections])
  ...
  const snapWheelToSection = useCallback(
    (event) => {
      const container = scrollRef.current
      ...
      if (isScrollSnapDisabled()) return false
  ...
  const suspendSnapForSidebarLayout = useCallback(() => {
    const container = scrollRef.current
    if (!container) return

    disableScrollSnap(SIDEBAR_LAYOUT_TOKEN, container)
    clearTimeout(sidebarLayoutTimeoutRef.current)
    sidebarLayoutTimeoutRef.current = setTimeout(() => {
      enableScrollSnap(SIDEBAR_LAYOUT_TOKEN, container)
    }, SIDEBAR_LAYOUT_LOCK_MS)
  }, [])
  ...
  useEffect(
    () => () => {
      clearTimeout(lockTimeoutRef.current)
      clearTimeout(sidebarLayoutTimeoutRef.current)
      enableScrollSnap(SIDEBAR_LAYOUT_TOKEN, scrollRef.current)
    },
    [],
  )
  ...
  return (
    ...
    >
      <Sidebar
        ...
        onLayoutTransition={suspendSnapForSidebarLayout}
        sections={sections}
      />
      <main
        ref={scrollRef}
