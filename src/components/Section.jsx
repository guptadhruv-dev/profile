...
const MARKDOWN_COMPONENTS = {
  ...
  a({ href, children, ...props }) {
    ...
    return (
      <a href={proxiedHref} {...externalLinkProps(proxiedHref)} {...domProps}>
        {children}
      </a>
    )
  },
  img({ src, ...props }) {
    const { node, ...domProps } = props
    return <img src={proxyProtectedUrl(src)} {...domProps} />
  },
}
...
export default function Section({ id, vars = {}, content, isFirst = false }) {
  ...
  return (
    <section id={id} className="section-pane" data-first={isFirst ? '' : undefined}>
